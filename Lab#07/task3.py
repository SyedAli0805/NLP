...
df_neu = df.loc[(df['Rating'] == 3) | (df['Rating'] == 4)].reset_index(drop=True)
# Match the number of neutral reviews with the negative reviews
df_neu = df_neu.loc[:len(df_neg) - 1]
# Match the number of positive reviews with the negative reviews
df_pos = df_five.loc[:len(df_neg) - 1]
# Combine positive and negative reviews
df_all = pd.concat([df_neg, df_neu, df_pos], axis=0).reset_index(drop=True)
# Add a Sentiments column with binary labels

df_all['Sentiments'] = np.where(
    df_all['Rating'] == 5, 
    'Positive', 
    np.where(
        (df_all['Rating'] == 3) | (df_all['Rating'] == 4), 
        'Neutral', 
        'Negative'
    )
)
...
rev_neu = ['The product is okay. It meets basic expectations but does not stand out in any way. Not too bad, not too great either.']
rev_neu_vec = v.transform(rev_neu)
prediction_neu = clf_svm.predict(rev_neu_vec)
print(f"Prediction for positive review: {prediction_neu[0]}")

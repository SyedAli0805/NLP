...
# F1 Score
fscore = f1_score(y_test, knn.predict(x_test_vec), average='weighted')
print(f"Weighted F1 Score: {fscore:.2f}")
# Classification report
print("\nClassification Report:")
print(classification_report(y_test, knn.predict(x_test_vec)))
# Test with custom reviews
rev_neg = ['Absolutely hate this place, horrible food']
rev_neg_vec = v.transform(rev_neg)
prediction_neg = knn.predict(rev_neg_vec)
...
prediction_pos = knn.predict(rev_pos_vec)
